    ...
    <div className="flex py-4 items-center justify-between 2xl:px-80 bg-white">
      ...
      <div className="flex justify-center items-center relative">
        ...
        <div className="group">
          ...
          <p className="absolute -top-2 text-xs -right-3 bg-red-500 text-white rounded-2xl px-1"  hidden={cart?.length === 0 }>{cart?.length}</p>
          </div>
          ...
          <div className="bg-neutral-50 p-2 shadow-md border rounded-sm absolute w-full h-96 top-10 left-0 overflow-y-scroll z-10 group-hover:block hidden">
            ...
            <p className="text-neutral-400 text-xs"  hidden={cart?.length === 0 }>{cart?.length} sản phẩm thêm vào giỏ hàng</p>
            {cart?.map((value, idx) => {
