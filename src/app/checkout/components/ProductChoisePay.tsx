...
export default function ProductChoisePay() {
    ...
    const getProductDetail = useCallback(async () => {
        if (typeof productId === "string" && productId) {
            try {
                ...
                if (product) {
                    setProduct(product)
                    const res = await fetch(`${NEXT_PUBLIC_LOCAL}/api/get/info-seller?seller-id=${product.data[0].seller_id}`, {
                        method: "GET",
                        headers: {
    ...
    return (
        product && infoSeller && <div className="bg-white p-6 rounded-xl mt-2 mb-10">
            ...
            <div className="grid grid-cols-6 my-6">
                <div className="col-span-6">
                    <p className="flex items-center">{infoSeller.data.nameShop} | <span className="flex text-xs justify-center items-center"><BsFillChatSquareTextFill className="mr-1 ml-4" />chat ngay</span></p>
                </div>
            </div>
            <div className="grid grid-cols-6 border-b">
                <div className="col-span-3">
                    <div className="flex my-2">
                        <Image src={`/do-tho/${product.data[0].img[0]}`} alt={`${product.data[0].name}`} width={48} height={48} />
                        <div>
                            <p className="text-ellipsis line-clamp-1 mx-2">{product.data[0].name}</p>
                        </div>
                    </div>
                </div>
                <div className="col-span-1">
                    <div className=" mt-2">{new Intl.NumberFormat("vi-VN", { style: "currency", currency: "VND" }).format(product.data[0].price)}</div>
                </div>
                <div className="col-span-1">
                ...
                <div className="col-span-1">
                    <div className="mt-2">
                        <p className="flex justify-end pr-16">{new Intl.NumberFormat("vi-VN", { style: "currency", currency: "VND" }).format((quantityChoise * product.data[0].price))}</p>
                    </div>
                </div>
            </div>
            ...
            <div className="flex justify-end pr-16">
                <div>
                    <p className="my-4">Tổng tiền hàng: <span className="ml-2">{new Intl.NumberFormat("vi-VN", { style: "currency", currency: "VND" }).format((quantityChoise * product?.data[0].price))}</span></p>
                    ...
                    <p>Tổng thanh toán:<span className="text-2xl text-red-500 ml-2">{new Intl.NumberFormat("vi-VN", { style: "currency", currency: "VND" }).format((quantityChoise * product?.data[0].price + totalMoneyShip))}</span> </p>
                </div>
            </div>
